...
/*
  ...
  double beta;
  const double alpha_th = 2;

  double alpha_long_t1 = alpha_long;
  double alpha_short_t1 = alpha_short;
  alpha_long = eta_long*total +(1-eta_long)*alpha_long_t1;
  alpha_short = eta_short*total +(1-eta_short)*alpha_short_t1;
  beta = 1 - alpha_th*alpha_short / alpha_long;
  //printf("%lf\t%lf\t%lf\t%lf\n",total,alpha_short,alpha_long,beta);
  printf("%lf\n",beta);
  if(beta > 0){
    printf("kidnaped\n");
  }
